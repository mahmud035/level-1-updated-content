...
const Registration = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      //* Step 1: Create New User (Firebase & MongoDB)
      const userCredential = await createUser(
        formData.email,
        formData.password
      );

      // For MongoDB (Extra for this project)
      ...
      //* Step 2: Update user's profile
      await updateUserProfile(formData.name, formData.photoURL);

      //* Step 3: Set updated user info into state
      setUser({
        ...userCredential.user,
      ...
      //* Step 4: Generate accessToken & refreshToken
      const user: ILoginData = { email: userCredential.user?.email || '' };
